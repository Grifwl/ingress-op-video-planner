...
- **Filename template**: `frame_####` — each `#` becomes one digit of the
  frame number (four `#` gives `0001.png`, `0002.png`, ...). Use this exact
  name: [`make_video.md`](make_video.md)'s commands are written against
  `frame_%04d.png` so they work unchanged, whatever operation you're
  rendering.
